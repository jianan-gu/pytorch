    ...
    if torch.compiler.is_dynamo_compiling():
        if return_lse:
            if query.device.type == "cpu":
                    raise NotImplementedError("`return_lse` is not supported yet on CPU device when using torch.compile.")

        if query.requires_grad:
            raise NotImplementedError("FlexAttention only supports inference on CPU device when using torch.compile.")
