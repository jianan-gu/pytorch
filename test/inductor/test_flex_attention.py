...
class TestFlexAttention(InductorTestCase):
    ...
    def run_test(
        ...
        q = torch.randn(
            (Q_B, Q_H, Q_S, Q_D), dtype=dtype, device=device, requires_grad=is_requires_grad
        )
        k = torch.randn(
    ...
    def test_strided_inputs(self, device: str, dtype: torch.dtype, q_s, k_s, v_s, do_s):
        if device == "cpu":
            ...
            is_requires_grad = True
        q1 = torch.randn((B * H * S * D * 2), dtype=dtype, device=device, requires_grad=is_requires_grad)
        k1 = torch.randn((B * H * S * D * 2), dtype=dtype, device=device, requires_grad=is_requires_grad)
        v1 = torch.randn((B * H * S * D * 2), dtype=dtype, device=device, requires_grad=is_requires_grad)
        do1 = torch.randn((B * H * S * D * 2), dtype=dtype, device=device, requires_grad=is_requires_grad)
        ...
        def coerce_to_strides(val, shape, strides):
            ...
            assert strides[-1] == 1
            return torch.as_strided(val, shape, strides, offset).requires_grad_(is_requires_grad)
    ...
    def test_load_from_view_buffer(self, device):
        dtype = test_dtypes_fast[device]
        W = 8
        is_test_inference = True if device == "cpu" else False
        ...
        q = torch.randn(B, H, H * W, D, device=device, dtype=dtype, requires_grad=not is_test_inference)
        k = torch.randn(B, H, H * W, D, device=device, dtype=dtype, requires_grad=not is_test_inference)
        v = torch.randn(B, H, H * W, D, device=device, dtype=dtype, requires_grad=not is_test_inference)
        out = m(q, k, v)
...
def forward(self, arg0_1, arg1_1, arg2_1, arg3_1, arg4_1):
    ...
    def test_fully_masked_out_rows(self, device: str, compile: bool):
        ...
        self.run_test(noop_mod, torch.float32, B, H, S, D, B, H, S, D, block_mask, device=device)
    ...
    def test_modular_indexing(self, device):
        ...
        m = Attention().cuda() if device=="cuda" else Attention()
        m = m.eval().to(dtype)
